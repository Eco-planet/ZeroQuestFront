    ...
    :class="myEntries.myEntriesArr.length>10 ? '': 'cardMediaBottom'">
      ...
      <div v-for="(item) in myEntriesOrder.slice(0,moreLimit)" :key="item.idx" class="mt-7 p-5 bg-white">
        ...
        <div class="flex justify-between">
          <svg @click="updataEntryData(item.image, item.title, item.desc)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
const updataEntryData = (entryImage, entryTitle, entryDesc) => {
  console.log("entryImage",entryImage)
  console.log("entryTitle",entryTitle)
  console.log("entryDesc",entryDesc)
